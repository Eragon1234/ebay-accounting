...
export async function getExpenses(
  ...
  return db.query.expense.findMany({
    limit,
    offset,
    orderBy: [desc(expense.date), asc(expense.name)],
  });
}
...
export async function getExpenseInRange(
  start: Date,
  end: Date,
): Promise<ExpenseSum> {
  const db = await getDbAsync();
  ...
  const result = await db
    .select({
      ...
      netto:
        sql`sum((${expense.amount} / (100 + ${expense.vat}) * 100))`.mapWith(
          Number,
        ),
      vat: sql`sum(${expense.amount} - (${expense.amount} / (100 + ${expense.vat}) * 100))`.mapWith(
        Number,
      ),
    })
    .from(expense)
...
export async function getExpenseInRangeByType(
  start: Date,
  end: Date,
): Promise<ExpenseSumByType[]> {
  const db = await getDbAsync();
...
export async function updateExpense(
  id: number,
  updatedFields: Partial<NewExpense>,
) {
  const db = await getDbAsync();
...
export async function deleteExpense(id: number): Promise<boolean> {
  ...
  if (deletedExpense.file) {
    try {
      ...
      console.error(
        `Failed to delete file '${deletedExpense.file}' for expense with ID ${id}:`,
        error,
      );
    }
  }
